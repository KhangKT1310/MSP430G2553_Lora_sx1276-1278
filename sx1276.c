...
void sx1276_init(radio_events_t *events)
{
  ...
  // Init irq interrupts
  P2DIR &= (~Dio0); // Set P2.1 SEL as Input
  P2IES &= (~Dio0); // Rising endge 0 -> 1
  P2IE |= (Dio0);   // Enable interrupt for P2.1
  P2IFG &= (~Dio0); // Clear interrupt flag for P2.1
...
void sx1276_rxchain_calibration()
{
  ...
  initialFreq = (uint32_t)((double)(((uint32_t)sx1276_read(REG_FRFMSB) << 16) |
...
uint32_t sx1276_get_timeonair(radio_modem_t modem, uint8_t pktLen)
{
  ...
  case MODEM_FSK:
  {
    airTime = (uint32_t)round((8 * (sx1276.Settings.Fsk.PreambleLen + ((sx1276_read(REG_SYNCCONFIG) & ~RF_SYNCCONFIG_SYNCSIZE_MASK) + 1) + ((sx1276.Settings.Fsk.FixLen == 0x01) ? 0.0 : 1.0) + (((sx1276_read(REG_PACKETCONFIG1) & ~RF_PACKETCONFIG1_ADDRSFILTERING_MASK) != 0x00) ? 1.0 : 0) + pktLen + ((sx1276.Settings.Fsk.CrcOn == 0x01) ? 2.0 : 0)) / sx1276.Settings.Fsk.Datarate) * 1e3);
  }
  break;
  case MODEM_LORA:
  {
  ...
  case MODEM_LORA:
  {
    ...
    if (sx1276.Settings.LoRa.Bandwidth < 9)
    {
      ...
      case 0: // 7.8 kHz
        ...
        sx1276_set_channel((uint32_t)(sx1276.Settings.Channel + 7.81e3));
        break;
      case 1: // 10.4 kHz
        sx1276_write(REG_LR_TEST2F, 0x44);
        sx1276_set_channel((uint32_t)(sx1276.Settings.Channel + 10.42e3));
        break;
      case 2: // 15.6 kHz
        sx1276_write(REG_LR_TEST2F, 0x44);
        sx1276_set_channel((uint32_t)(sx1276.Settings.Channel + 15.62e3));
        break;
      case 3: // 20.8 kHz
        sx1276_write(REG_LR_TEST2F, 0x44);
        sx1276_set_channel((uint32_t)(sx1276.Settings.Channel + 20.83e3));
        break;
      case 4: // 31.2 kHz
        sx1276_write(REG_LR_TEST2F, 0x44);
        sx1276_set_channel((uint32_t)(sx1276.Settings.Channel + 31.25e3));
        break;
      case 5: // 41.4 kHz
        sx1276_write(REG_LR_TEST2F, 0x44);
        sx1276_set_channel((uint32_t)(sx1276.Settings.Channel + 41.67e3));
        break;
      case 6: // 62.5 kHz
